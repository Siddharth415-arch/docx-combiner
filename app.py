...
Four tools in one:
  ...
  4. CSV to Text Converter      — converts one or more .csv files to .txt at once
"""
...
# ══════════════════════════════════════════════════════════════════════
#  TOOL 4 — CSV TO TEXT CONVERTER
# ══════════════════════════════════════════════════════════════════════
 
CSV_REQUIRED_COLUMNS = ['episode_name', 'episode_number', 'summary']
 
 
def convert_single_csv_to_text(csv_bytes: bytes, filename: str) -> str:
    """
    Convert a single CSV's bytes into the episode-summary .txt format.
 
    Expects columns: episode_name, episode_number, summary.
    Raises ValueError with a clear message if the CSV can't be read or is
    ...
    for _, row in df.iterrows():
        ...
        header = f"Episode {ep_num}: {ep_name}"
        lines.append(header)
...
def render_csv_to_text_converter():
    st.title("📄 CSV to Text Converter")
    st.markdown(
        "Upload one or more `.csv` files and this tool will convert **all of "
        "them at once** into `.txt` files, matching each episode's number, "
        "name, and summary."
    )
    st.markdown(
        "**Required columns:** `episode_name`, `episode_number`, `summary`"
    )
    st.divider()
    ...
    uploaded_files = st.file_uploader(
        ...
        type=["csv"],
        accept_multiple_files=True,
        key="csv2txt_uploader",
        help="Select all the CSV files at once — they'll all be converted in one go.",
    )
    st.divider()
    ...
    if not st.button("🔄 Convert All to Text", type="primary", use_container_width=True):
        return
    ...
    for i, uf in enumerate(uploaded_files):
        ...
        try:
            text_content = convert_single_csv_to_text(uf.getvalue(), uf.name)
            out_name = f"{Path(uf.name).stem}.txt"
            results.append((text_content, out_name))
        except Exception as e:
            errors.append((uf.name, str(e), traceback.format_exc()))
    ...
    if len(results) == 1:
        text_content, out_name = results[0]
        st.download_button(
            label=f"⬇️ Download {out_name}",
        ...
        st.markdown("**Download individual files:**")
        for text_content, out_name in results:
            st.download_button(
                label=f"⬇️ {out_name}",
                data=text_content.encode("utf-8"),
                file_name=out_name,
                mime="text/plain",
                use_container_width=True,
                key=f"csv2txt_dl_{out_name}",
            )
        st.markdown("**Or download everything at once:**")
        zip_buf = io.BytesIO()
        with zipfile.ZipFile(zip_buf, "w", zipfile.ZIP_DEFLATED) as zf:
            for text_content, out_name in results:
                zf.writestr(out_name, text_content.encode("utf-8"))
        st.download_button(
            label=f"📦 Download all {len(results)} files as ZIP",
            data=zip_buf.getvalue(),
            file_name="csv_to_text_converted.zip",
            mime="application/zip",
            use_container_width=True,
            type="primary",
        )
    ...
    st.divider()
    st.caption(
        "Each row becomes: `Episode <number>: <name>` followed by a dashed "
        "underline and the summary. Output filename matches the input CSV's name."
    )
...
elif selected_tool == "🌐 Document Translator":
    render_document_translator()
else:
    render_csv_to_text_converter()
